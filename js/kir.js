...
function refreshRuanganDropdown() {
...
function renderArsipList() {
  ...
  tbody.innerHTML = arsip
    .map((k) => {
      const ruangan = getRuanganById(k.ruanganId);
      return `
        <tr>
...
document.getElementById("btn-simpan-identitas").addEventListener("click", () => {
  saveIdentitas(collectIdentitasForm());
...
function tampilkanKartu() {
  ...
  const barangRuangan = getInventarisByKartu(ruanganId, tahun, semester);
