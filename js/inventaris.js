...
function refreshRuanganOptions() {
  const ruanganList = getRuanganList();
...
function refreshTahunKirFilterOptions() {
  const tahunTersedia = getDistinctTahunKirList();
...
function refreshKatalogOptions() {
...
function renderTabel() {
  const joined = getInventarisJoined();
...
selectKatalog.addEventListener("change", () => {
  const aset = getAsetById(selectKatalog.value);
  if (!aset) return;
  ...
  const jumlahDipakai = jumlahPemakaianAset(aset.id);
  if (jumlahDipakai > 0) {
...
function openModalEdit(id) {
  const inv = getInventarisById(id);
  if (!inv) return;
  const aset = getAsetById(inv.asetId);
  if (!aset) return;
...
formBarang.addEventListener("submit", (e) => {
  ...
  if (asetId) {
    updateAset(asetId, dataAset);
  } else {
    asetId = addAset(dataAset);
  }
  ...
  if (invId) {
    updateInventaris(invId, dataInventaris);
    showToast("Barang berhasil diperbarui.");
  } else {
    addInventaris(dataInventaris);
    showToast("Barang berhasil ditambahkan.");
  }

  closeModalBarang();
  refreshRuanganOptions();
  refreshTahunKirFilterOptions();
  renderTabel();
});

/* ---------- lihat detail ---------- */
function openModalLihat(id) {
  const joined = getInventarisJoined();
...
tbody.addEventListener("click", (e) => {
  ...
  if (btn.dataset.action === "hapus") {
    const inv = getInventarisById(id);
    const aset = inv ? getAsetById(inv.asetId) : null;
